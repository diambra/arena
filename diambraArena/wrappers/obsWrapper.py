        ...
        if isinstance(v, dict):
            ...
            if isinstance(vSpace, spaces.MultiDiscrete):
                ...
                observation[k] = (np.array(observation[k]).astype(
                    np.float32) - lowVal) / (highVal - lowVal)
    return observation
...
class ActionsStack(gym.Wrapper):
    def __init__(self, env, nActionsStack, nPlayers=1):
        ...
        for iPlayer in range(self.nPlayers):
            self.moveActionStack.append(
                deque([0 for i in range(nActionsStack)], maxlen=nActionsStack))
    ...
    def step(self, action):
        ...
        # Add noAction for nActionsStack - 1 times
