...
class diambraImitationLearning(gym.Env):
    ...
    def reset(self):
        ...
        if self.playersNum == "2P": # e.g. HUMvsHUM
            ...
            # First reset for this trajectory
                ...
                self.RLTrajDict["addObs"] = self.AddObsCorrection(self.RLTrajDict["addObs"],
                                                                  player_id=0)
    ...
    def AddObsCorrection(self, addObs, player_id):
        ...
        for observation in addObs:
            ...
            nScalarAddPar = additionalPar - 2*self.nChars - 2*self.actBufLen*\
                                (self.n_actions[0]+self.n_actions[1])
            addPar = observation[:,:]
            ...
            # P1
            if player_id == 0:
                newAddObs[1:1+newAdditionalPar] = np.append( np.append( np.append( 
                    np.append(moveActionsP1, attackActionsP1), [P1Health, P2Health, 
                                                                P1Position, P2Position]),
                                                                        P1Char), P2Char)
            # P2
            else:
                newAddObs[1:1+newAdditionalPar] = np.append( np.append( np.append(
                    np.append(moveActionsP2, attackActionsP2), [P2Health, P1Health, 
