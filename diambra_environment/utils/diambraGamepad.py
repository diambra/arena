...
class diambraGamepad(Thread): # def class typr thread
   def __init__(self, actionList=( ("NoMove", "Left", "UpLeft", "Up", "UpRight",
      ...
      self.gamepadConfigFileName = os.path.join(homeDir, '.diambra/config/gamepadConfig.cfg')
      ...
      print("Initializing gamepad # {}, named: {}".format(self.gamepadNum, self.gamepad))
      ...
      print("Diambra GamePad initialized on device # {}, named: {}".format(self.gamepadNum, self.gamepad))
   ...
   def processGamepadDictForSave(self):
       ...
       for key1, value1 in self.gamePadDict["Absolute"].items():
           ...
           if "ABS_HAT0" in key1:
               ...
               # Analog moves
               absListDictAnalog.append([key1, value1[0], value1[1], value1[2], value1[3]])
   ...
   def loadAllGamepadConfigurations(self):
       ...
       try:
           ...
           print("No gamepad configuration file found in: {}".format(os.path.join(homeDir, '.diambra/config/')))
       ...
       for cfgFileDict in self.cfgFileDictList:
           if cfgFileDict["name"] == self.gamepad.name:
               ...
               try:
                   ...
                   self.gamePadDict["Absolute"] = defaultdict(lambda: defaultdict(lambda: [[], 0]))
                   self.gamePadDict["Absolute"]["ABS_HAT0Y"] = defaultdict(lambda: [[], 0]) # No-op action = 0
                   self.gamePadDict["Absolute"]["ABS_HAT0X"] = defaultdict(lambda: [[], 0]) # No-op action = 0
                   ...
                   for item in cfgFileDict["absoluteValuesListDigital"]:
                       self.gamePadDict["Absolute"][item[0]][item[1]] = [item[2], abs(item[1])]

                   # Read "Absolute" info ANALOG
                   for item in cfgFileDict["absoluteValuesListAnalog"]:
                       self.gamePadDict["Absolute"][item[0]] = [item[1], item[2], item[3], item[4]]

                   configFound = True
                   print("Gamepad configuration file found in: {}".format(os.path.join(homeDir, '.diambra/config/')))
                   print("Gamepad configuration file loaded.")
               except:
                   print("Invalid gamepad configuration file found in: {}".format(os.path.join(homeDir, '.diambra/config/')))

       if not configFound:
           print("Configuration for this gamepad not present in gamepad configuration file")
   ...
   def configure(self):
       ...
       for idx in range(8):
           ...
           while butNotSet:
               ...
               for event in self.gamepad.read():
                   if event.ev_type == "Key":
                       if event.code != self.startCode and event.code != self.selectCode:
                           if event.state == 1:
       ...
       self.gamePadDict["Absolute"]["ABS_HAT0Y"] = defaultdict(lambda: [[], 0])
       self.gamePadDict["Absolute"]["ABS_HAT0X"] = defaultdict(lambda: [[], 0])
       self.gamePadDict["Absolute"]["ABS_HAT0Y"][0] = [[0, 2], 0]
       self.gamePadDict["Absolute"]["ABS_HAT0X"][0] = [[1, 3], 0]
                   ...
                   if event.ev_type == "Absolute":
                       if event.code == "ABS_HAT0" + eventCodesList[idx]:
                           if abs(event.state) == 1:
                               print("{} move event code = {}, event state = {}".format(move, event.code, event.state))
                               self.gamePadDict["Absolute"][event.code][event.state] = [idx, abs(event.state)]
                           elif event.state == 0:
                               butNotSet = False
                           else:
                               print("Digital Move Stick assumes not admissible values: {}".format(event.state))
                               print("Digital Move Stick not supported, configuration skipped")
                               endFlag = True
                               break
                   else:
                       if event.code == self.startCode or event.code == self.selectCode:
                           if event.state == 0:
       ...
       for idx, move in enumerate(movesList):
           ...
           print("Move left analog in {} position, keep it there and press Start".format(move))
       ...
       for idx in range(2):
           self.originAnalogVal[movesList[idx]] = int( (self.maxAnalogVal[movesList[idx]] + self.maxAnalogVal[movesList[idx+2]]) / 2.0 )
           self.originAnalogVal[movesList[idx+2]] = self.originAnalogVal[movesList[idx]]
       ...
       self.deltaPerc= {}
       for idx in range(2):
           self.deltaPerc[idx] = (self.maxAnalogVal[movesList[idx]] - self.originAnalogVal[movesList[idx]]) * threshPerc
       ...
       # Addressing Y-X axis
       for idx in range(2):
           print("{} move event code = ABS_{}, ".format(movesList[idx], eventCodesList[idx]) +\
                 "event state = {}".format(self.maxAnalogVal[movesList[idx]]))
           print("{} move event code = ABS_{}, ".format(movesList[idx+2], eventCodesList[idx+2]) +\
                 " event state = {}".format(self.maxAnalogVal[movesList[idx+2]]))
           print("NO {}-{} move event code =".format(movesList[idx], movesList[idx+2]) +\
           ...
           if self.deltaPerc[idx] > 0:
               ...
               print("Not admissible values found in analog stick configuration, skipping")
   ...
   def configTest(self):
       ...
       while True:
           ...
           if actions[0] != 0:
               print("Move action = {}. (Press START to end configuration test).".format(self.allActionsList[0][actions[0]]))
           if actions[1] != 0:
               print("Attack action = {}. (Press START to end configuration test).".format(self.allActionsList[1][actions[1]]))
           if actions[2] != 0:
               break
   ...
   def run(self):      # run is a default Thread function
      while True:   # loop for ever
         for event in self.gamepad.read():   # check events of gamepads, if not event, all is stop
            if event.ev_type == "Key":   # category of binary respond values
               ...
               # Start
                  ...
                  self.startBut = event.state
                   ...
                   self.eventHashAttack[self.gamePadDict[event.ev_type][event.code]] = event.state

            elif "ABS_HAT0" in event.code:   # category of move values (digital moves)
            ...
            elif event.code == "ABS_X" or event.code == "ABS_Y":   # category of move values (analog left stick)
